...
export default function PaymentMethodFormModal({
  ...
  const validate = () => {
    const newErrors = {};
    if (!form.nombre.trim()) newErrors.nombre = "El nombre del método de pago es obligatorio.";
          ...
          <div className="modal-header border-0 px-4 pt-4 pb-0" style={{ background: "#fff" }}>
            <div>
              <h5 className="fw-bold mb-1" style={{ color: "#001219" }}>
                {isEditing ? "✏️ Editar Método de Pago" : "+ Nuevo Método de Pago"}
              </h5>
              <p className="text-muted mb-0" style={{ fontSize: 14 }}>
                {isEditing
                  ? "Modificá los datos del método de pago."
                  : "Completá los campos para agregar un método de pago."}
              </p>
            </div>
              ...
              <div className="row g-3">
                ...
                <div className="col-12">
                  ...
                  <textarea
                    ...
                    name="descripcion"
                    placeholder="Breve descripción del método de pago (opcional)..."
                    rows={2}
                ...
                {/* Datos / Instrucciones */}
                <div className="col-12">
                  <label className="form-label fw-semibold" style={{ fontSize: 14 }}>Datos / Instrucciones</label>
                  <textarea
                    className="form-control"
                    name="datos"
                    placeholder="Ej: Alias: bellaimagen.mp | Titular: Aldana"
                    rows={2}
                    value={form.datos}
                    onChange={handleChange}
                    style={{ borderRadius: 10, padding: "10px 14px", fontSize: 14, border: "1px solid #e0e0e0", resize: "vertical" }}
                  />
                  <div className="text-muted mt-1" style={{ fontSize: 12 }}>
                    Información que se mostrará al cliente (alias, CBU, instrucciones, etc.).
                  </div>
                </div>
              </div>
            </div>
