 ...
 * ShippingMethodTable
 * ─────────────────────────────────────────────────
 * Tabla responsive de métodos de envío con búsqueda/filtro.
 *
 * Props:
...
export default function ShippingMethodTable({
  ...
  const filtered = useMemo(() => {
    if (!search.trim()) return shippingMethods;
    ...
    <section>
      ...
      {!loading && !error && (
        ...
        >
          ...
          <span className="fw-semibold">
            Total de métodos:{" "}
            <span style={{ color: "#ee9b00" }}>{shippingMethods.length}</span>
          </span>
          {search.trim() && (
      ...
      {!loading && !error && (
        <div className="d-none d-md-block">
            ...
            >
              <thead>
                <tr style={{ background: "#e9d8a6", color: "#333" }}>
                  ...
                  <th style={{ padding: "14px 16px", fontWeight: 600 }}>Costo</th>
                  <th style={{ padding: "14px 16px", fontWeight: 600 }}>Tiempo</th>
                  <th style={{ padding: "14px 16px", fontWeight: 600 }}>Gratis desde</th>
              ...
              <tbody>
                {filtered.map((method) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span className="fw-semibold">
                        {method.costo === 0 ? "Gratis" : formatPrice(method.costo)}
                      </span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <span style={{ color: "#555", fontSize: 13 }}>
                        {method.gratisDesde > 0 ? formatPrice(method.gratisDesde) : "—"}
                      </span>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
          ...
          >
            <span>
              Mostrando {filtered.length} de {shippingMethods.length} métodos
            </span>
            {search.trim() && (
              <button
                type="button"
                ...
                <div className="card-body p-3">
                  <div className="d-flex gap-3">
                    ...
                    <div className="flex-grow-1 min-w-0">
                      ...
                      {method.gratisDesde > 0 && (
                        <p className="mb-0" style={{ fontSize: 12, color: "#888", lineHeight: 1.4 }}>
                          <strong>Envío gratis desde:</strong> {formatPrice(method.gratisDesde)}
                        </p>
                      )}
                    </div>
                  </div>
